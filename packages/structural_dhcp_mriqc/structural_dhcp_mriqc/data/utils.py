...
def _fetch_file(url, dataset_dir, filetype=None, resume=True, overwrite=False,
    ...
    if temp_downloads is None:
        temp_downloads = op.join(op.expanduser('~/.cache/structural_dhcp_mriqc'), 'downloads')
...
def _get_dataset_dir(dataset_name, data_dir=None, default_paths=None,
    ...
    .. note::
      ...
      the following priority :
        ...
        5. ~/.cache/structural_dhcp_mriqc in the user home folder
    ...
    if data_dir is None:
        ...
        paths.append((op.expanduser('~/.cache/structural_dhcp_mriqc'), False))
    ...
    raise OSError('structural_dhcp_mriqc tried to store the dataset in the following '
