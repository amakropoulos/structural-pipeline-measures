...
def anat_qc_workflow_dhcp1(name='MRIQC_Anat', settings=None):
    ...
    inputnode = pe.Node(niu.IdentityInterface(
        fields=['bids_root', 'subject_id', 'session_id', 'run_id',
        'reorient', 'bias_corrected', 'bias',
        'tissue_segmentation', 'in_pvms']), name='inputnode')
    ...
    workflow.connect([
        ...
        (inputnode, measures, [('reorient', 'in_file'), ('bias_corrected', 'in_noinu'),
                               ('bias', 'in_bias'), ('tissue_segmentation', 'in_segm'), 
                               ('in_pvms', 'in_pvms') ]),
