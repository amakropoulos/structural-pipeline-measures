...
for l in 0 ${corts};do
  ...
  #surface area
  oSa=`surface-scalar-statistics $outer $mask -name drawem |grep "^Area  "|tr -d ' ' |cut -d':' -f 2 | sed -e 's/[eE]+*/\\*10\\^/'`
  pSa=`surface-scalar-statistics $pial  $mask -name drawem |grep "^Area  "|tr -d ' ' |cut -d':' -f 2 | sed -e 's/[eE]+*/\\*10\\^/'`
  if [ "$oSa" == "" -o "$pSa" == "" ];then 
    GI[$r]="-";
  else
